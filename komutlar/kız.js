...
await member.setNickname(`● ${isim} | ${yas}`)
  member.roles.add("913916916173799474"); // Kız rol ID'si
  member.roles.remove("913916915540455464"); //(Unregister) Rolünün ID'si
     const endlesskanal = message.guild.channels.cache.find(c => c.id == "913917088303833158") //Chat kanalının ID'
    const embed1 = new Discord.MessageEmbed() 
    .setDescription(`<@!${member.id}> **Aramıza <@&913916916173799474> olarak katıldı sıcak bir hoşgeldin diyelim!** \`${member.guild.memberCount}\` **Üyeye Ulaştık**`) 
    .setColor("RANDOM")
  let embed = new Discord.MessageEmbed() 
  .setColor("66FF00")                                                           
  .setTimestamp()
  .setDescription(`<@!${member.id}> <@&913916916173799474> olarak kayıt edildi!`) 
  .setFooter(`Lrows`)
